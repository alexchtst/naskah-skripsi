...
\section{Rancangan Sistem}
	...
	Pada penelitian ini, representasi \emph{fixed-point} menggunakan jumlah bit pecahan
	atau \emph{fractional bit} sebesar $F=14$. Dengan konfigurasi tersebut, nilai
	\emph{floating-point} dikalikan dengan faktor skala $2^{14}$, kemudian dibulatkan
	dan dibatasi agar tetap berada pada rentang bilangan bertanda 16-bit. Pemakaian
	\emph{fractional bit} ini penting karena nilai bobot CNN umumnya berada pada
	rentang kecil, sehingga dibutuhkan resolusi pecahan yang cukup halus agar informasi
	parameter tidak banyak hilang setelah proses kuantitasi. Secara umum, proses
	konversi dari nilai real $x$ ke representasi \emph{fixed-point} 16-bit dapat
	...
	karena jumlah bit pecahan yang terlalu kecil dapat memperbesar galat kuantitasi,
	sedangkan jumlah bit pecahan yang terlalu besar dapat mempersempit rentang nilai
	integer yang dapat direpresentasikan \cite{Lin2016FixedPointQuantization}.
	...
	frame membutuhkan kapasitas yang lebih besar.
	...
	buffer, ROI, CNN accelerator, argmax, dan VGA display.
...
risiko kesalahan integrasi perangkat keras.

    \subsection{Implementasi Model CNN di Python}
	Model CNN terlebih dahulu dibangun dan dilatih menggunakan Python. Implementasi ini berfungsi sebagai baseline software untuk menentukan arsitektur, bobot, bias, serta keluaran referensi pada setiap layer. Hasil dari tahap ini digunakan sebagai acuan ketika model dipindahkan ke representasi fixed-point dan Verilog.
    \subsection{Implementasi Kuantisasi Fixed-Point}
	Parameter hasil pelatihan dikonversi dari floating-point ke fixed-point 16-bit dengan fractional bit yang telah ditentukan. Proses ini dilakukan sebelum parameter dimasukkan ke file memori Verilog. Validasi kuantisasi dilakukan dengan membandingkan rentang nilai, jumlah saturasi, dan kesesuaian keluaran terhadap model Python.
    \subsection{Implementasi CNN ke Verilog}
	Implementasi Verilog merealisasikan operasi konvolusi, ReLU, pooling, fully connected, dan argmax sebagai rangkaian perangkat keras. Blok komputasi menggunakan kombinasi MAC berbasis DSP, shift register, FIFO, FSM, dan mekanisme valid-ready. Setiap layer diuji terhadap keluaran Python agar perbedaan akibat kuantisasi dan lebar bit dapat diamati secara bertahap.
    \subsection{Implementasi OV7670}
	Implementasi OV7670 mencakup konfigurasi sensor, pembacaan sinyal sinkronisasi, dan penyusunan data piksel. Pengujian awal dilakukan untuk memastikan bahwa sinyal VSYNC, HREF, pixel clock, dan data paralel dapat diterima secara stabil sebelum data kamera dihubungkan ke jalur DDR.
    \subsection{Implementasi VGA}
	Implementasi VGA dilakukan dengan membangkitkan timing horizontal, timing vertikal, sinyal HSYNC, VSYNC, dan data warna RGB. Pengujian sanitasi menggunakan pola warna dilakukan untuk memastikan bahwa jalur tampilan bekerja sebelum digunakan untuk menampilkan data dari frame buffer.
    \subsection{Implementasi DDR sebagai Frame Buffer}
	DDR diimplementasikan sebagai frame buffer melalui MIG dan transaksi AXI. Jalur tulis menerima data dari kamera, sedangkan jalur baca menyediakan data untuk VGA dan ROI. Tahap ini penting karena kamera, CNN, dan VGA tidak selalu bekerja pada kebutuhan waktu yang sama.
...
\section{Metode Pengujian}
	...
	Pengujian kapasitas FPGA dilakukan dari laporan implementasi Vivado. Parameter yang diamati meliputi LUT, FF, BRAM, dan DSP karena keempat resource tersebut menentukan kelayakan integrasi pada Arty A7-100T.
...
\section{Metode Evaluasi}
	...
	Evaluasi akurasi digunakan untuk menilai kesesuaian hasil klasifikasi terhadap label data uji dan sebagai pembanding antara baseline software dan implementasi hardware.
    \subsection{Evaluasi Perbedaan Bit}
	Evaluasi perbedaan bit digunakan untuk melihat dampak representasi fixed-point terhadap nilai antara pada setiap blok CNN. Evaluasi ini penting karena kesalahan kecil pada layer awal dapat terakumulasi pada layer berikutnya.
    \subsection{Evaluasi Latensi Inferensi}
	Evaluasi latensi inferensi dihitung berdasarkan jumlah siklus clock yang diperlukan sampai keluaran CNN tersedia. Nilai ini digunakan untuk menilai kemampuan sistem terhadap kebutuhan real-time.
